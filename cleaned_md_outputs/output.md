...
11 for `2 × 2 × 2,20` for `3 × 3 × 3, Θ(n<sup>2</sup> / n)` for `n × n × n` [Demaine, Demaine, Eisenstat Lubiw Winslow 2011]
...
\# vertices `=8!· 3<sup>8</sup>=264,539,520` where 8 ! comes from having 8 cubelets in arbitrary positions and `3<sup>8</sup>` comes as each cubelet has 3 possible twists.
...
- for each vertex `u ∈ V, A d j[u]` stores `u` 's neighbors, i.e., `\{v ∈ V |(u, v) ∈ E\} . (u, v)` are just outgoing edges if directed. (See Fig. 2 for an example.)
...
frontier `<sub>n</sub>=\{s\}`
frontier `<sub>1</sub>=\{a, x\}`
frontier `<sub>2</sub>=\{z, d, c\}`
frontier `<sub>3</sub>=\{f, v\}`
(not `x, c, d)`
...
```math
 ...
 time =<sub>v ∈ V</sub>|A d j[V]|=\{{array}{l}
